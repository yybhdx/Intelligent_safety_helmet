// MPU6050传感器的DMP（Digital Motion Processor）功能相关的头文件定义
// 代码服务对象
// 这段代码是为MPU6050六轴传感器（三轴加速度计+三轴陀螺仪）的DMP功能服务的。从代码中的#define MPU6050和大量DMP相关的宏定义可以看出，这是InvenSense公司提供的官方DMP固件接口定义。

// 主要用途
// 这个头文件定义了DMP功能所需的所有寄存器地址和配置参数，主要用于：

// 姿态解算：通过四元数计算欧拉角（俯仰角、横滚角、偏航角）
// 运动检测：包括敲击检测、运动方向识别等
// 数据融合：融合加速度计和陀螺仪数据
// 功能配置：配置DMP的各种功能开关和参数
// 代码结构分析
// 代码主要分为几个部分：

// 配置键定义（KEY_CFG_*）：用于配置DMP的各种功能
// 数据寄存器定义（KEY_D_*）：定义DMP内部数据存储地址
// 陀螺仪和加速度计校准（KEY_D_GYRO_BIAS/KEY_D_ACC_BIAS）
// 四元数相关（KEY_D_CS_TQ*）：用于姿态解算
// 手势识别（Gesture Keys）：实现敲击等手势检测
// 计步器功能（Pedometer keys）：用于计步功能
// 电子图像稳定（EIS keys）：用于图像稳定功能
/*
 $License:
