...
// ?职责?：MPU6050芯片内部有一个小处理器（DMP），非常擅长做复杂的数学计算。
// 这个部门的工作就是把一段编译好的“固件”（dmp_memory数组，就像一套武功秘籍）加载到DMP里，然后管理它。
// DMP帮我们完成了最难的姿态解算（从加速度和角速度算出角度），大大减轻了单片机的负担。
// 它提供了 dmp_load_motion_driver_firmware（加载固件）、dmp_read_fifo（读取DMP处理后的数据）等函数。
